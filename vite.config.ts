...
export default defineConfig({
  ...
  fmt: {
    ...
    // CHANGELOG.md is machine-generated by release-please on every release and
    // committed verbatim via the release PR (see release-please-config.json;
    // @semantic-release/changelog wrote it before the batched-release switch);
    // its markdown is not prettier-clean (extra blank lines around headings),
    // so formatting it would make `vp run check` in CI fail on the release
    // commit's output. Exclude it — cdk-local/cdkd achieve the same by scoping
    // fmt to src only.
