...
const readIamPolicy: OverrideReader = async ({ declared, region }) => {
  ...
  if (role) doc = (await c.send(new GetRolePolicyCommand({ RoleName: role, PolicyName: name }))).PolicyDocument;
...
const readIamManagedPolicy: OverrideReader = async ({ physicalId, region }) => {
  ...
  const ver = (await c.send(new GetPolicyVersionCommand({ PolicyArn: physicalId, VersionId: p.DefaultVersionId }))).PolicyVersion;
