...
# key. Re-measured at this tree, broken arm, 81 rows:
#
#   broken `rebase-merge` arm, clean global config    76 pass / 5 fail
#   ...and with a global `rebase.backend = apply`     76 pass / 5 fail   same
#
...
#
#   global `commit.gpgsign = true`                    56 pass / 25 fail
...
#
#   GIT_TEMPLATE_DIR=<dir with a failing hook>        56 pass / 25 fail
#   GIT_CONFIG_COUNT / _KEY_0 / _VALUE_0              56 pass / 25 fail
#   GIT_DIR=<elsewhere>                               49 pass / 32 fail
#
...
# 43 pass / 38 fail under `HOOK_BASH=/bin/bash` and 81 pass / 0 fail under
...
# damage under 3.2 and the clean 5.x total. Unlike the two sibling repos, no doc
# here carried a competing pair to notice the drift against -- which is the
# weaker position, not the safer one: nothing disagreed because nothing else
# had been measured. Numbers that are cheap to leave behind are exactly the
# ones to re-measure whenever rows are added; this round did, and the pair
# above is that measurement.
#
...
#   20 rows want exit 0 and EXIT BEFORE the broken construct is
...
spaced_repo="$FIXROOT/main repo with spaces"
