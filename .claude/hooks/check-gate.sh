...
# Since go-to-k/cdk-real-drift#1837 hand-written *.md files are `check` inputs
# too, because tests/markdown-fmt-corruption-1771.test.ts reads every tracked
# *.md bar CHANGELOG.md. Two buckets:
#   README.md / DESIGN.md / docs/**  -> BOTH gates (they are also docs' subject)
#   CLAUDE.md / CONTRIBUTING.md / demo/README.md / .claude/rules/** /
#     CHANGELOG.md -> `check` ALONE (the docs gate includes only src/**,
#     docs/**, README.md, DESIGN.md). CHANGELOG.md is the one file that scanner
#     excludes, but it joined the `check` gate at the release-please splice fix:
#     tests/release-please-v0.test.ts reads it to fence the shape the Changelog
#     updater splices into (go-to-k/cdkd#2503).
# The error message names the skill(s) to re-run.
#
# cdk-real-drift is a smaller repo than cdkd, so only a subset of cdkd's
# gate suite is wired. .claude/settings.json registers 11 distinct PreToolUse
# hooks -- this one plus branch-gate, bughunt-clean-gate, ci-green-gate,
# deploy-autoarm-gate, issue-classification-label-gate, issue-dup-check-gate,
# non-english-text-gate, stale-base-gate, verify-pr-gate and worktree-guard --
# and 2 Stop hooks (stop-cleanup-warn, stop-unmerged-lane-warn). The
