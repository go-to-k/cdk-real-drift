...
# *.md bar CHANGELOG.md. Three buckets, not two:
#   README.md / DESIGN.md / docs/**  -> BOTH gates (they are also docs' subject)
#   CLAUDE.md / CONTRIBUTING.md / demo/README.md / .claude/rules/** -> `check`
#     ALONE (the docs gate includes only src/**, docs/**, README.md, DESIGN.md)
#   CHANGELOG.md -> NEITHER. It is machine-generated by release-please and is
#     the one file that scanner excludes, so it is deliberately out of scope.
