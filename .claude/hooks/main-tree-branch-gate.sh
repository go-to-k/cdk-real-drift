...
# standing between the walk and a word whose expansion this gate cannot see. A
# library predating it makes the `if ! gate_word_is_literal ...` call exit 127,
# which `!` reads as TRUE, so every word would look unaccountable and the gate
# would block everything -- loud, but a gate that refuses `git checkout main` is
# as unusable as one that allows a switch. Refusing here says why.
