...
# markers and an armed bug-hunt sentinel. A MEASUREMENT RECORD, not a roster:
# only the third gate still exists, and rewriting the table would falsify what
# was observed.
#
#   gate                    plain   -R form
...
# `gh -R o/r pr merge` is simply wrong. The equality is asserted per verb in
# `_command-match.test.sh` ("THE BYPASS CASES"); keep a new verb regex built from
# `GATE_GH_C` rather than a hand-rolled `-C`-only absorber.
#
