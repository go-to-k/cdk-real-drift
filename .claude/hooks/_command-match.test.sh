...
# every merge gate of the day (each measured plain rc=2, `-R` rc=0). Of those,
# `bughunt-clean-gate` is the one still standing, and it is SUPPOSED to match a
# `-R` merge: leaked AWS resources are leaked whichever repo the flag names. This
# assertion was INVERTED from `want_match 1` — an earlier revision pinned the old
# behaviour as intentional scoping, which was wrong.
