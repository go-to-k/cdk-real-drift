...
# the fuller shape that idiom is usually written with are deliberately NOT used
# here, because each was measured to buy nothing at this call site:
#
#   NO MEMO. A per-SEGMENT caller memoises the last answer in a pair of globals.
#   This gate asks at most once per command, so a memo would cache nothing.
#
#   NO `canonicalize`. A caller that compares a RAW `<dir>` argument -- a payload
#   cwd, which may still carry a symlink -- against the porcelain path must
#   resolve both. This gate compares `$target_top`, which git has ALREADY
...
if [ -z "$branch" ]; then
  ...
  #
  # (b) is reachable from the documented flow: nothing refuses `git checkout
  # <sha>` in the main checkout, the tree detaches, and this gate then waved a
  # commit straight into the SHARED main checkout. Measured on a scratch
  # opted-in repo before this arm existed, driving this hook with a
  ...
  #
  # BLOCKED ONLY IN THE MAIN CHECKOUT. A detached HEAD in a LINKED worktree is a
  # documented wind-down step -- a session that must not remove its worktree
  # runs `git switch --detach origin/main`, because a worktree with no current
  ...
  # path containing a SPACE is truncated
  ...
  if [ "$target_top" = "$main_checkout" ]; then
    ...
    # that was ALREADY detached, which is one `git checkout <sha>` away.
