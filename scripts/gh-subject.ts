 ...
 *
 * The check that consumes it (`check-gh-body-english.ts`) replaces a PreToolUse
 * bash hook. Such a hook is handed the raw SHELL COMMAND TEXT of a `gh`
 * invocation and has to locate the body inside it, which means reimplementing
 * pieces of a shell parser --
 *
 ...
 * never interpolated into a shell command with a `github.event...` expression.
 * A body containing `"; rm -rf ...` is then just bytes in a JSON document. The
 ...
 * ported check can be LINE-anchored. Normalising here rather than in the check
 * keeps a second consumer from drifting on it.
 */
...
export interface Subject {
  ...
  body: string;
  /**
   * Current label names. No check reads them yet; the workflow supplies them so
   * a later consumer does not have to change the document shape.
   */
  labels: string[];
 ...
 * body yields an empty string, which is scanned and is clean.
 */
export function parseSubject(json: string): Subject {
  ...
  if (kind !== 'issue' && kind !== 'issue_comment' && kind !== 'pull_request') {
    throw new Error(
      `subject.kind must be issue | issue_comment | pull_request, got ${String(kind)}`
    );
  }
  ...
  // field it would then have to remember not to scan is how that rule gets lost.
