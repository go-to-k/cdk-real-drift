/**
 * CI port of `.claude/hooks/non-english-text-gate.sh`, deleted with the rest of
 * the PreToolUse gates that did not clear the blocking criterion
 * (go-to-k/cdk-real-drift#1917).
 *
 * WHAT THE HOOK WAS
 * -----------------
 ...
 * `tests/non-english-class-sync.test.ts`, which compares source and
...
export function main(env: NodeJS.ProcessEnv = process.env): number {
  ...
  console.error('  - For a verbatim session quote: rewrite it as a project-level');
  console.error('    contract statement, not as a quote.');
