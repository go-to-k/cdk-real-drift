 ...
 *
 * The issue / PR / comment half of the English-only rule. cdk-real-drift never
 * had a body-side PreToolUse gate; this is ported from the sibling repo cdkd,
 * whose `.claude/hooks/gh-body-english-gate.sh` it replaced, so that both
 * surfaces the rule names -- the DIFF and the PUBLISHED TEXT -- are covered.
 *
 * ## What is PRESERVED
 *
 ...
 *     the check can only report it and ask for an edit. Stated here rather than
 *     left to be discovered.
 *   - Release notes (`gh release create --notes` / `--notes-file`) were in the
 *     hook's verb set and are NOT covered here. Releases are cut by
 *     release-please from commit messages, which the PR-diff check reads as
 *     part of the branch, so the gap is narrow -- but it is a gap.
 *
 ...
 * and "by eye" is not the guarantee: `tests/non-english-class-sync.test.ts`
...
export function formatReport(subject: Subject, offenders: Offender[]): string {
  ...
  lines.push('');
  lines.push('Issue and PR bodies, titles, comments and release notes are public OSS');
