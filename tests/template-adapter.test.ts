...
import { buildResolverContext, collectRolesWithSiblingPolicies, parseTemplateBody } from "../src/desired/template-adapter.js";
...
describe("buildResolverContext", () => {
  it("merges template defaults with deployed params (deployed wins) + sets pseudo", () => {
    const template = { Parameters: { Env: { Default: "dev" }, Other: { Default: "x" } }, Conditions: { C: true } };
    const ctx = buildResolverContext(template, { Env: "prod" }, { Log: "phys" }, "us-west-2", "999", "S", "arn:stack");
