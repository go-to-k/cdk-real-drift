...
ROOT="$(cd "$HERE/../../.." && pwd)"
cd "$HERE"
STACK=CdkRealDriftIntegEcsTaskDefCaps
...
CDKRD_CORPUS_DIR="/tmp/corpus-ecs-taskdef-caps" $CLI check "$STACK" --region "$REGION" --verbose 2>&1 | tee "/tmp/cdkrd-$STACK.pre.out" || true
