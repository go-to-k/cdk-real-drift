...
# MUST be CLEAN. A RegexPatternSet with multiple RegularExpressionList entries declared
# in a non-sorted order — WAFv2 stores the regex list as a set and echoes it in its own
# order; any declared-tier drift here is a set-ordering FP (sibling of the suppressed
# IPSet.Addresses, R84).
set -uo pipefail
HERE="$(cd "$(dirname "$0")" && pwd)"
ROOT="$(cd "$HERE/../../.." && pwd)"
cd "$HERE"
STACK=CdkRealDriftIntegWafv2RegexPatternSet
