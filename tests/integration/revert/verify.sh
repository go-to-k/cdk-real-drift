...
#   deploy -> record (baseline) -> check CLEAN
...
# Enable acceleration BEFORE record so the baseline records AccelerationStatus=Enabled.
...
echo "=== enable acceleration, then record (baseline) ==="
...
echo "=== inject DECLARED drift (suspend versioning) + UNDECLARED drift (suspend accel from recorded Enabled) ==="
