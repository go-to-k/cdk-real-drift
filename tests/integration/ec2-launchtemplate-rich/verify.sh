...
# MUST be CLEAN. EC2 LaunchTemplate LaunchTemplateData is read via the SDK override; any
...
CDKRD_CORPUS_DIR="/tmp/corpus-ec2-launchtemplate-rich" $CLI check "$STACK" --region "$REGION" --verbose 2>&1 | tee "/tmp/cdkrd-$STACK.pre.out" || true
