#!/usr/bin/env bash
# cdkdrift basic integration test (real AWS).
...
$CLI check "$STACK" --region "$REGION" | tee /tmp/cdkdrift-integ.out
rc=${PIPESTATUS[0]}
[ "$rc" -eq 1 ] || fail "expected drift exit 1, got $rc"
grep -q "AccelerateConfiguration" /tmp/cdkdrift-integ.out || fail "AccelerateConfiguration not reported"
