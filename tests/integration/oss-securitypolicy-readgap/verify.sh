...
STACK=CdkRealDriftIntegOssSecurityPolicyReadGap; REGION="${AWS_REGION:-us-east-1}"; CLI="node $ROOT/dist/cli.js"
...
grep -qE "skipped=[1-9]" "/tmp/cdkrd-prerecord-$STACK.out" && fail "expected every resource READ (skipped=0)"
...
rc=${PIPESTATUS[0]}; [ "$rc" -eq 0 ] || fail "expected CLEAN, got $rc"
