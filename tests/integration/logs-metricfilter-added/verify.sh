...
#      reports the filter under [Potential Drift] and is NOT drift (exit 0) -> `record`
...
grep -q "Potential Drift" /tmp/cdkrd-integ-lmf.out || fail "added filter not under [Potential Drift]"
...
grep -q "Potential Drift" /tmp/cdkrd-integ-lmf-clean.out && fail "still Not-Recorded after record (GetResource likely failed)" || true
