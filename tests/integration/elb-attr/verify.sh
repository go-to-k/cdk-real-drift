...
# ELBv2 LoadBalancer/TargetGroup attribute bags: after the fail-closed fix the ~20
# server-default attributes are recorded as undeclared inventory, so record->check
# must still be CLEAN (no FP) once they are snapshotted.
set -uo pipefail
