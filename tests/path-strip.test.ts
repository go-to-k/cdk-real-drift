...
describe('deepStripPaths', () => {
  ...
  it('strips a nested path with * over array elements', () => {
    const o: Record<string, unknown> = { Rules: [{ Id: 1, Transition: 'x' }, { Id: 2, Transition: 'y' }] };
...
describe('parseSchema nested paths', () => {
  it('keeps nested writeOnly out of the top-level set but in writeOnlyPaths', () => {
    const info = parseSchema(JSON.stringify({
      ...
      writeOnlyProperties: ['/properties/AccessControl', '/properties/LifecycleConfiguration/Rules/*/Transition'],
    }));
