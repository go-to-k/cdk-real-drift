...
// the shape a spawning suite hits on a loaded CI runner (~2 s local, 5000 ms
// in CI), and the one that reads as flakiness rather than an under-declared
// bound). The bound's job is to stop a HANG, not to police latency, so it
// is set generously.
...
function runCli(
  ...
  const dir = mkdtempSync(join(tmpdir(), 'cdkrd-gh-body-english-'));
...
describe('the character class', () => {
  ...
  it('includes both endpoints of all five declared ranges', () => {
    const endpoints = [
      0x3000,
      0x303f, // CJK punctuation
      0x3040,
      0x309f, // hiragana
      0x30a0,
      0x30ff, // katakana
      0x4e00,
      0x9fff, // CJK ideographs
      0xac00,
      0xd7af, // hangul
    ];
...
describe('which fields are scanned', () => {
  ...
  it('does not invent a title for a comment', () => {
    ...
    expect(
      scanSubject({ kind: 'issue_comment', number: 5, title: KANJI, body: 'English.', labels: [] })
    ).toEqual([]);
  });
...
describe('the report', () => {
  ...
  it('names the offending characters, de-duplicated and in order', () => {
    expect(offendingCharacters(`a${CJK_COMMA}b${CJK_COMMA}c\u3002`)).toEqual([CJK_COMMA, '\u3002']);
...
// The RENDERED report is NOT asserted here. cdkd's copy of this suite parses the
// report with `marked` and checks that each offender stays inside ONE list and
// that an all-backtick payload renders as one code block. `marked` is not a
// dependency of this repo and one is not added for a test, so what survives is
// the SOURCE-level contract above: the fence is longer than the longest backtick
// run, the payload is folded to one line, and the fence is indented two spaces
// so it stays inside its list item. The renderer-level gap is recorded in
// docs/tooling-backlog.md as a first occurrence.
