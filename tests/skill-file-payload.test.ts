...
const SPLIT_SKILLS: Record<string, { minFiles: number; minCorpusBytes: number }> = {
  ...
  // follow-up that dropped "a NEW fixture" as a `next` reason paid its
  // triage.md growth the same way (three sentences trimmed in section 3-b):
  // corpus 143,901 B, `corpus - largest` 117,992, 8 B of room. The next
  // non-leader addition must be paid for in its own file.
