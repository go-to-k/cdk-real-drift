...
// the shape a spawning suite hits on a loaded CI runner: it reads as flakiness
// rather than as an under-declared bound. The bound's job is to stop a HANG,
// not to police latency, so it is set generously.
...
/**
 * Port of `.claude/hooks/non-english-text-gate.test.sh` to the CI check that
 * replaced the hook (go-to-k/cdk-real-drift#1917 deleted the hook and its
 * harness).
 *
...
describe("the shipped allow-list and the check's own sources", () => {
  ...
  it.each([
    ...
    // The TEST files are exactly where someone pastes a fixture character, and
    // the cost of that is every later PR touching them being unopenable until
    // an allow-list entry is added.
    'tests/pr-non-english-text.test.ts',
    'tests/gh-body-english.test.ts',
    'tests/non-english-class-sync.test.ts',
    'tests/annotation-text.test.ts',
...
function makeRepo(files: Record<string, string>): Repo {
  const dir = mkdtempSync(join(tmpdir(), 'cdkrd-noneng-'));
...
function runCheck(repo: Repo, overrides: Record<string, string> = {}) {
  try {
    // Plain `node`: this repo pins Node 24 (`.node-version`), which strips type
    // annotations by default, and the CI job runs the script the same way. A
    // runner whose node cannot strip types fails LOUDLY here.
